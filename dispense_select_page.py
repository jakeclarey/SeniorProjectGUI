...
class DispenseSelectPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Top buttons
        tk.Button(self, text="Exit", font=("Arial", 14), width=12, height=2,
                  bg="white", command=lambda: controller.show_frame("IdlePage")).place(x=10, y=10)
        tk.Button(self, text="View Available Stock", font=("Arial", 14), width=20, height=2,
                  bg="white").place(x=170, y=10)
        ...
        # Title
        tk.Label(self, text="Select Hardware and Quantity", font=("Arial", 20),
                 bg="#0032A0", fg="white").place(relx=0.5, y=100, anchor="center")

        # Hardware buttons
        hardware_items = [
            "Screw: M2 x 10", "Screw: M3 x 8", "Screw: M3 x 16", "Screw: M3 x 30", "Screw: M3 x 50",
            "Nut: M2", "Nut: M3", "Locknut: M3", "Washer: M3"
        ]
        ...
        for i, item in enumerate(hardware_items):
            ...
            x_pos = (x_base_row1 if row == 0 else x_base_row2) + col * (btn_width * 9 + x_padding)
            y_pos = y_start_row1 if row == 0 else y_start_row2

            btn = tk.Radiobutton(self, text=item, indicatoron=0, width=btn_width, height=btn_height,
                                 font=("Arial", 12), bg="white", fg="black",
                                 variable=self.selected_hardware, value=item)
            btn.place(x=x_pos, y=y_pos)

        # Quantity slider
        self.quantity = tk.IntVar(value=5)
        self.slider = ttk.Scale(self, from_=1, to=10, orient="horizontal", variable=self.quantity)
        self.slider.place(relx=0.5, y=340, width=700, anchor="center")

        self.slider_value_label = tk.Label(self, text=str(self.quantity.get()), font=("Arial", 14),
                                           bg="#0032A0", fg="white")
        self.slider_value_label.place(relx=0.5, y=300, anchor="center")
        self.slider.bind("<Motion>", lambda e: self.slider_value_label.config(text=str(int(self.quantity.get()))))

        # Finished button
        tk.Button(self, text="Finished", font=("Arial", 14), width=20, height=2,
                  bg="white", command=self.on_finish).place(relx=0.5, y=450, anchor="center")
    ...
    def show_shopping_list(self):
        ...
        result = messagebox.askyesnocancel("Submit Order", f"{summary}\n\nSubmit this order?")

        if result is None:
            return  # Cancel
        elif result is False:
            clear = messagebox.askyesno("Clear Cart", "Clear shopping list and start over?")
            if clear:
            ...
            if total_credits > self.credits:
                messagebox.showerror("Insufficient Credits", "You don't have enough credits.")
                self.shopping_list.clear()
                self.hardware_names.clear()
                return
            ...
            with open('hardware_list.txt', 'w') as f:
                entries = [f"{hw.replace(':', '')}:{qty}" for hw, qty in self.shopping_list]
