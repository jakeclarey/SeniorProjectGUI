...
class AdminMenuPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Title
        tk.Label(
            self, text="Admin Menu", font=("Arial", 24), fg="white", bg="#0032A0"
        ...
        tk.Button(
            ...
            width=5,
            height=2,
            command=lambda: self.adjust_stock("inc"),
        ...
        tk.Button(
            ...
            width=5,
            height=2,
            command=lambda: self.adjust_stock("dec"),
        ).grid(row=6, column=1, pady=(10, 0))

        # Bottom-right control buttons
        tk.Button(
            self,
            text="Reset All Stock to 200",
            font=("Arial", 14),
            command=self.reset_all_stock,
        ).place(relx=1, rely=1, anchor="se", x=-20, y=-20)
        tk.Button(
            self,
            text="Clear Keycard Entries",
            font=("Arial", 14),
            command=self.clear_keycard_entries,
        ).place(relx=1, rely=1, anchor="se", x=-20, y=-70)
        tk.Button(
            self,
            text="Set Selected to 0",
            font=("Arial", 14),
            command=lambda: self.adjust_stock("zero"),
        ).place(relx=1, rely=1, anchor="se", x=-20, y=-120)
