...
class PreSortPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(self, text="Return to Activity", font=("Arial", 14),
                           width=18, height=2,
                           command=lambda: self.controller.show_frame("ActivityPage"))
        button.place(x=10, y=10)  # Top-left position
        
        # Button in top-left corner
        button = tk.Button(self, text="Press to Continue", font=("Arial", 14),
                           width=18, height=2,
        ...
        project_text = tk.Label(box_frame, text="Please place parts in the intake and close the lid.", font=("Arial", 18), bg="white")
    ...
    def check_intake_closed(self):
        ...
        if result:
            ...
            self.after(0, lambda: self.status_label.config(text="Lid Was Closed - Continuing to Sorting"))
            self.after(1000, lambda: self.controller.show_frame("SortHardwarePage"))
        else:
            print("Lid was open")
            self.after(0, lambda: self.status_label.config(text="Lid Was Open - Please Close It and Try Again"))
