...
class DISPENSE_LOADING_SCREEN(tk.Toplevel):
    def __init__(self):
        super().__init__()
        ...
        user_id_decimal = int(self.user_id) if isinstance(self.user_id, str) else self.user_id

        user_info_frame = tk.Frame(self, bg="#598baf")
        user_info_frame.pack(fill="x")

        tk.Label(user_info_frame, text=f"User ID: {user_id_decimal}",
                 font=("Helvetica", 14), bg="#598baf", fg="black").pack(side="left", padx=20, pady=5)

        self.credits_label = tk.Label(user_info_frame, text=f"Credits: {self.credits}",
                                      font=("Helvetica", 14), bg="#598baf", fg="black")
        self.credits_label.pack(side="right", padx=20, pady=5)

        # Center canvas area
        canvas = tk.Canvas(self, width=1024, height=540, bg="#0032A0", highlightthickness=0)
        canvas.pack()
        ...
        try:
            ...
            canvas.create_text(512, 450, text="LOGO NOT FOUND", font=("Arial", 16), fill="black")

        # Message
        message = "Please Wait While System Processes Order"
        canvas.create_text(512, 200, text=message, font=("Montserrat", 20), fill="black")
